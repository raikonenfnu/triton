...
LogicalResult Pingponger::unmergeAndHoistAsyncWait(OpBuilder &builder,
  ...
  if (asyncWaitOps[0]->getNumOperands() != 2)
    return failure();
  LDBG("Pass step -1 on determining indices.");

  // Step 0: Determine asyncToken and it's corresponding memory/loadOp, by
  // tracking their arg index in the forOp.
  SmallVector<ttg::AsyncCommitGroupOp> prologueAsyncCommitOps;
  SmallVector<std::pair<int64_t, int64_t>> correspondTokenAndMemory;
  for (auto asyncToken : asyncWaitOps[0].getAsyncToken()) {
    ...
    if (!blockArg) {
      LDBG("blockArg cast failure.");
      return failure();
    }
    int64_t tokenArgIdx = blockArg.getArgNumber();
    ...
    if (!prologueAsyncCommitOp) {
      // TODO: Currently seem like it's the memref.load
      LDBG(initArg);
      LDBG("prologueAsyncCommitOp casting failure.");
      return failure();
    }
    ...
    if (!maybeLoadIterArgIdx.has_value()) {
      LDBG("maybeLoadIterArgIdx failure.");
      return failure();
    }
    int64_t loadIterArgIdx = maybeLoadIterArgIdx.value();
    correspondTokenAndMemory.emplace_back(tokenArgIdx, loadIterArgIdx);
  }
  LDBG("Pass step 0 on determining indices.");
  ...
  for (auto [tokenIdx, loadIdx] : correspondTokenAndMemory) {
    Value loadIterArg = forOp.getRegionIterArg(loadIdx);
  ...
  LDBG("Pass step 2 on using previous iteration token iterArg instead of "
       "current asyncWait.");
  ...
  for (ttg::AsyncCommitGroupOp asyncCommitOp : asyncCommitOps) {
    ...
    asyncCommitOp.getResult().replaceUsesWithIf(asyncWait, [&](OpOperand &use) {
      return use.getOwner() != asyncWait.getDefiningOp();
    });
  }

  // Step 4: Transform epilogue from having `async_wait + local_load` to ->
  // local_load, since we have moved
  //         the last `async_wait` to inside the loop.
  for (auto [tokenIdx, loadIdx] : correspondTokenAndMemory) {
    Value resultLoad = forOp.getResult(loadIdx);
  ...
  LDBG("Pass step 4 transforming epilogue.");
